    ...
    def place_entities(self,room,entities,max_monsters_per_room,max_items_per_room):
        ...
        for i in range(number_of_monsters):
            ...
            if not any([entity for entity in entities if entity.x == x and entity.y == y]):
                if randint(0,100)<80:
                    fighter_component=Fighter(hp=10,defence=0,power=3,max_items_per_room=35)
                    ...
                    fighter_component=Fighter(hp=16,defence=1,power=4,max_items_per_room=100)
                    ai_component=BasicMonster()
        ...
        for i in range(number_of_items):
            ...
            if not any([entity for entity in entities if entity.x==entity.y==y]):
                ...
                if item_chance<70:
                    ...
                    item_component=Item(use_function=cast_fireball,targeting=True,targeting_message=Message("Left-click a target tile for the fireball, or right-clikc to cancel.",libtcod.light_cyan),damage=12,radius=3)
                    item=Entity(x,y,"#",libtcod.red,"Fireballe Scroll",render_order=RenderOrder.ITEM,item=item_component)
                elif item_chance<90:
